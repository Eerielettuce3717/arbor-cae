...
export function ArViewportPlaceholder({
  ...
  return (
    <div className="pointer-events-auto absolute bottom-10 left-3 z-20 w-64 rounded-md border border-eng-border bg-eng-panel/95 p-3 shadow-xl backdrop-blur-sm">
      <div className="mb-2 flex items-start justify-between gap-2">
        <div>
          <p className="text-[11px] font-semibold text-eng-text">
            Augmented Reality
          </p>
          <p className="mt-0.5 text-[10px] leading-snug text-eng-muted">
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="text-eng-faint hover:text-eng-text"
      ...
      <p className="mt-2 font-mono text-[9px] text-eng-faint">
  ...
  return (
    <button
      ...
      className={`rounded border px-2.5 py-1.5 text-left transition-colors ${
        disabled
          ? "cursor-not-allowed border-eng-border/50 bg-eng-bg/40 text-eng-faint"
          : "border-sky-800/60 bg-eng-active/80 text-sky-200 hover:border-sky-600"
      }`}
    >
